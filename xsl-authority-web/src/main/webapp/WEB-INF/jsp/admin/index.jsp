        ...
        $.ajax({
            ...
            success :function (result) {
                ...
                if(result.status == 200){
                    ...
                    var adminPage = result.data;
                    var admins = adminPage.datas;
                    $.each(admins, function(i, admin){
                        tableContent +='<tr>';
                        tableContent +='    <td>'+(i+1)+'</td>';
                        tableContent +='    <td><input type="checkbox" name="adminid" value="'+admin.id+'"></td>';
                        tableContent +='    <td>'+admin.id+'</td>';
                        tableContent +='    <td>'+admin.name+'</td>';
                        tableContent +='    <td>'+admin.sex+'</td>';
                        tableContent +='    <td>'+admin.phone+'</td>';
                        tableContent +='    <td>'+admin.email+'</td>';
                        tableContent +='    <td>'+admin.createdate+'</td>';
                        tableContent +='    <td>'+admin.updatedate+'</td>';
                        tableContent +='    <td>';
                        tableContent +='        <button type="button" onclick="goAssignPage('+admin.id+')" class="btn btn-success btn-xs"><i class=" glyphicon glyphicon-check"></i></button>';
                        tableContent +='        <button type="button" onclick="updateMessage('+admin.id+')" class="btn btn-primary btn-xs"><i class=" glyphicon glyphicon-pencil"></i></button>';
                        tableContent +='        <button type="button" onclick="deleteMessage('+admin.id+',\''+admin.name+'\')" class="btn btn-danger btn-xs"><i class=" glyphicon glyphicon-remove"></i></button>';
                        tableContent +='    </td>';
                        tableContent +='</tr>';
                    });
                    ...
                    for ( var i = 1; i <= adminPage.totalno; i++ ) {
                        if ( i == pageno  ) {
